...
function crateIndex(tedad = 4) {
    let indexs = [];
    for (let index = 0; index < tedad; index++) {
        indexs.push(0);
    }
    return indexs;
}
function jameCArtha(shomarIf = 1, indexs = crateIndex(4), cartaJugada, misCartas, cartasDeMesa, misGanancias) {
    for (indexs[shomarIf - 1] = 0; indexs[shomarIf - 1] < indexs.length; indexs[shomarIf - 1]++) {
        let ift = 0;
        let ifs = shomarIf + 1;
        for (let index = 0; index < ifs - 1; index++) {
            if (cartasDeMesa.cartas[indexs[shomarIf - 1]].numero < 11) ift++;
        }
        let cart_s = 0;
        for (let index = 0; index < ifs - 1; index++) {
            cart_s += cartasDeMesa.cartas[shomarIf - 1].numero;
        }
        if (misCartas.cartas[cartaJugada].numero + cart_s == 11) ift++;
        if(shomarIf > 1) {
...
function ContarCartas(cartaJugada = 2, misCartas = {}, cartasDeMesa = {}, misGanancias = {}) {
    ...
    if (misCartas.cartas[cartaJugada].numero < 11) {
        jameCArtha(1, crateIndex(cartasDeMesa.cartas.length), cartaJugada, misCartas, cartasDeMesa, misGanancias);
